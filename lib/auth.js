...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, user, trigger, account }) {
      ...
      // Re-fetch plan from DB on explicit update() OR if cached value is >5 min old.
      // This ensures admin/Stripe plan changes propagate without requiring sign-out.
      const FIVE_MIN = 5 * 60 * 1000;
      const stale = !token.planFetchedAt || (Date.now() - token.planFetchedAt) > FIVE_MIN;
