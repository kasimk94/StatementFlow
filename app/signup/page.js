...
function getStrength(pw) {
  ...
  const hasUpr = /[A-Z]/.test(pw);
  if (pw.length >= 12 && hasNum && hasSym && hasUpr)
    return { level: 4, label: "Strong", color: "#22c55e" };
  if (pw.length >= 10 && hasNum && hasSym)
    return { level: 3, label: "Good", color: T.gold };
  if (pw.length >= 8 && (hasNum || hasSym))
        ...
        {[1, 2, 3, 4].map((i) => (
          <div
            ...
            style={{
              ...
              height: 3,
              borderRadius: 2,
              background: i <= level ? color : "#1E2A3A",
...
// ── Shared right panel (identical to login) ───────────────────────────────────
function RightPanel() {
  const features = [
    {
